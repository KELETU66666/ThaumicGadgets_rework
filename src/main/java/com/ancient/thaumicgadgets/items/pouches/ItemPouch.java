...
public class ItemPouch extends ItemBase {
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entity, int itemSlot, boolean isSelected) {
        if (!stack.hasTagCompound()) {
            ...
            nbt.setUniqueId("playerID", this.defaultUUID);
        ...
        if (stack.getTranslationKey().contains("magic") && stack.getTranslationKey().contains("pouch") && stack.getTranslationKey().contains("hungry") && !entity.isSneaking()) {
            ...
            for (EntityItem item : entity.world.getEntitiesWithinAABB(EntityItem.class, aabb)) {
                if (item != null) {
                    double x = entity.posX - item.posX;
                    double y = entity.posY - item.posY;
        ...
        if (!worldIn.isRemote) {
            ...
            if (!stack.getTranslationKey().contains("ender") && stack.getTranslationKey().contains("pouch")) {
                if (!playerIn.isSneaking()) {
                    ...
                    nbt.setUniqueId("playerID", playerIn.getUniqueID());
                    stack.setTagCompound(nbt);
                    playerIn.sendMessage(new TextComponentString(I18n.format("item.pouch.description.soulbinded")));
                } else if (nbt.getUniqueId("playerID").equals(playerIn.getUniqueID())) {
                    nbt.setUniqueId("playerID", this.defaultUUID);
                    stack.setTagCompound(nbt);
                    playerIn.sendMessage(new TextComponentString(I18n.format("item.pouch.description.unbinded")));
                } else {
    ...
    public NonNullList<ItemStack> getInventory(ItemStack stack) {
        ...
        NBTTagList items = nbt.getTagList(tagName, 10);
        NonNullList<ItemStack> inventory = NonNullList.withSize(18, ItemStack.EMPTY);
